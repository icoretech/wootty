...
| --- | --- | --- |
| FR-1 Connection lifecycle | `apps/server/internal/protocol/protocol_test.go` + `apps/server/internal/config/config_test.go` (attach contract + runtime config parsing) | `apps/web/e2e/terminal.spec.ts` -> "renders terminal UI and reaches connected state" |
