...
| FR-3 Heartbeat and stale transport detection | `apps/web/test/unit/lib/terminal-session.test.ts` (message parser for pong), `apps/web/test/unit/app/orchestrators/transport-orchestrator.test.tsx` (heartbeat timeout + reconnect) | covered by reconnect stability checks in e2e reconnect and resize scenarios |
